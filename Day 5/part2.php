<h1>Advent Of Code - Day 5, Part 2</h1>
...
if ($file) {
  ...
  while (($line = fgets($file)) !== " 1   2   3   4   5   6   7   8   9 \n") {
    ...
    if ($nbLigne === 1) {
      for ($i = 0; $i < count($cranes); $i++) {
        $stacks[] = array();
        if (trim($cranes[$i]) != "") { $stacks[$i][] = $cranes[$i]; }
      }
    } else {
      for ($i = 0; $i < count($cranes); $i++) {
        if (trim($cranes[$i]) != "") { array_unshift($stacks[$i], $cranes[$i]); }
      }
    }
  ...
  while (($line = fgets($file)) !== false) {
    ...
    if ($arrangement[0] == "move") {
      ...
      $movedCrane = array_splice($stacks[$from], -$nbCrane);
      $stacks[$to] = array_merge($stacks[$to], $movedCrane);
    }
  }
